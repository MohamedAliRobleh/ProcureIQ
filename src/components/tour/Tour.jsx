...
function tooltipStyle(rect, placement) {
  ...
  if (placement === 'right') {
    return {
      ...
      left: Math.min(rect.left + rect.width + gap, window.innerWidth - cardWidth - gap),
    }
  }
  if (placement === 'bottom') {
    return {
      top: Math.min(rect.top + rect.height + gap, window.innerHeight - cardHeight - gap),
      left: Math.max(gap, rect.left),
    }
  }
  if (placement === 'top') return { top: Math.max(gap, rect.top - 180), left: Math.max(gap, rect.left) }
  return { top: Math.max(gap, rect.top), left: Math.max(gap, rect.left) }
}

export default function Tour() {
  const { isOpen, stepIndex, next, back, close } = useTour()
  const step = TOUR_STEPS[stepIndex]
  // Call hook unconditionally; it tolerates null/undefined selector.
